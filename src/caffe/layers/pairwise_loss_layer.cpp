...
void PairwiseLossLayer<Dtype>::Reshape(
  ...
  focal_.Reshape(1, 1, outer_num_, outer_num_);
  own_similarity_.Reshape(1, 1, outer_num_, outer_num_);
  weight_vector_.Reshape(1,1,1, outer_num_);
  neg_weight_vector_.Reshape(1,1,1, outer_num_);
  distance_.Reshape(1, 1, outer_num_, outer_num_);
}
