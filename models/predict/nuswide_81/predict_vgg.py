...
def get_codes_and_labels(params):
    ...
    return dict(database_code=np.array(database_code), database_labels=np.array(database_labels), validation_code=np.array(validation_code), validation_labels=np.array(validation_labels))
...
if __name__ == "__main__":
    ...
    database_lines = open("./data/nuswide_81/database_new.txt", "r").readlines()
    test_lines = open("./data/nuswide_81/test_new.txt", "r").readlines()
    ...
    if args.code_path:
        ...
        for i in range(nthreads):
            params.append(dict(model_file="./models/predict/deploy_vgg1.prototxt",
